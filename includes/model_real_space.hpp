//
//  model_real_space.hpp
//
//
//  Created by Francisco Brito on 26/04/2019.
//
...
public:
    model(int tmd, double on_site_interaction, double chem_pot, double beta_f)
    {
        t0 = -1;
        // Define model
        ...
        TB_Hamiltonian = new Eigen::Matrix<std::complex<double>, -1, -1>;
...
void model::TMDnanoribbon()
{
    ...
    Eigen::Matrix<std::complex<double>, -1, -1>::Zero
      (NX * NY * NORB, NX * NY * NORB);
    for (int x = 0; x < NX; x++ )
        for (int y = 0; y < NY; y++ )
          for (int orb = 0; orb < NORB; orb++ )
          {
              unsigned start_idx = orb + NORB * ( NY * x + y );
                  ...
                  neighbor_idx < n.NHoppings[orb]; neighbor_idx++)
              {
                  ...
                  unsigned end_idx =
                    ...
                    * ( NY * end_x + end_y );
                  int cell_distance =
                    (x + n.x_idxs[orb].at(neighbor_idx) + NX) / NX - 1;
                  if ( end_y >= 0 && end_y < NY )
                  {
...
Eigen::MatrixXd model::MFbandsUp()
{
    for (int idx_k = 0; idx_k < NK; idx_k++)
    {
        for (int idxEn = 0; idxEn < NA * NY * NORB; idxEn++)
...
Eigen::MatrixXd model::MFbandsDw()
{
    for (int idx_k = 0; idx_k < NK; idx_k++)
    {
        for (int idxEn = 0; idxEn < NA * NY * NORB; idxEn++)
...
void model::compute_grand_potential()
{
    double dH;
    double OmegaMF = 0.0;
    if (beta == beta_target)
...
void model::save_grand_potential(unsigned it)
{
    energies(it) = energy;
}

double model::grand_potential()
{
    return energy;
}

Eigen::VectorXd model::grand_potential_evol()
{
    return energies;
}

void model::anneal(unsigned it)
{
    if (beta < BETA_THRESHOLD)
    {
        if (beta < beta_target)
...
bool model::loop_condition(unsigned it)
{
...
Eigen::VectorXd model::UpField()
{
    return nUp;
}

Eigen::VectorXd model::DwField()
{
    return nDw;
}

double model::filling()
{
    return ( nUp.sum() + nDw.sum() ) / NA / NY / NORB;
}

void model::init_random(int seed)
{
