...
#define BETA_THRESHOLD 100
#endif
#ifndef DELTA
#define DELTA 0.0000051
#endif
#ifndef DAMP_FREQ
#define DAMP_FREQ 1
#endif
...
int main(int argc, char **argv)
{
    ...
    model_real_space solverLow(tmd, u, muLow, beta);
    ...
    {
        model_real_space solverLowRd(tmd, u, muLow, beta);
    ...
    model_real_space solverHigh(tmd, u, muHigh, beta);
    ...
    for (int indSols=0; indSols < noSeeds; indSols++)
    {
        model_real_space solverHighRd(tmd, u, muHigh, beta);
    ...
    if ( (bestElDensLow - fill_target) * (bestElDensHigh - fill_target) < 0 )
    {
        while ( it_bissect < it_bissect_max )
        {
            ...
            for (int indSols=0; indSols < noSeeds; indSols++)
            {
                model_real_space solverRd(tmd, u, mu, beta);
